...
the search difficult.'''),
    ...
    def user_bodyweight(self, weight):
        ...
        if (not WeightEntry.objects.filter(user=self.user).exists()
            or (datetime.date.today()- WeightEntry.objects.filter 
              (user=self.user).latest().date
...
class DaysOfWeek(models.Model):
    ...
    This model is needed so that 'Day'
    can have multiple days of the week selected
    '''
